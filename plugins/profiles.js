...
async function handleRegisterCommand(medoContext) {
    ...
    if (medoExistingTitle) {
        const medoExistingUser = await medoContext.conn.getName(`${String(medoExistingTitle.userId)}@s.whatsapp.net`);
        // Tag the user by including their JID in the mentions array.
        medoContext.reply(
            `اللقب ${medoTitle} مأخوذ من طرف @${medoExistingUser}`,
            { mentions: [`${String(medoExistingTitle.userId)}@s.whatsapp.net`] }
        );
    } else {
...
async function handleGetTitleCommand(medoContext) {
    ...
    if (medoQuotedUserTitle) {
        const medoQuotedUserName = await medoContext.conn.getName(`${String(medoUserId)}@s.whatsapp.net`);
        medoContext.reply(
            `┇ لقب @${medoQuotedUserName} هو : ${medoQuotedUserTitle.bk9}`,
            { mentions: [`${String(medoUserId)}@s.whatsapp.net`] }
        );
    } else {
...
async function handleCheckTitleCommand(medoContext) {
    ...
    if (medoCheckResult) {
        const medoCheckUser = await medoContext.conn.getName(`${String(medoCheckResult.userId)}@s.whatsapp.net`);
        medoContext.reply(
            `اللقب ${medoCheckTitle} مأخوذ من طرف @${medoCheckUser}`,
            { mentions: [`${String(medoCheckResult.userId)}@s.whatsapp.net`] }
        );
    } else {
...
medoHandler.command = ['الالقاب', 'تسجيل', 'لقبي', 'لقبه', 'حذف_لقب', 'لقب'];
